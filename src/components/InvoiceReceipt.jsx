...
const COMPANY_LOGO_URL = "/launcherdesk_logo.svg"; // null → "S" monogram
  ...
  const raw =
    Array.isArray(invoice.lineItems) && invoice.lineItems.length > 0
      ...
      : [
          {
            type: "plan",
            name: invoice.planName || "Launcherdesk CRM Subscription",
            sub:
...
export default function InvoiceReceipt({ invoice, company: companyProp, onClose }) {
  ...
  const company = {
    name:    "LAUNCHERDESK DIGITAL SOLUTIONS LLP",
